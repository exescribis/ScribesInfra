...
from pyuseocl.analyzer import UseOCLModelFile
...
from evaluation \
    import ModelValidation, InvariantViolation, \
    InvariantValidation, \
...
class UseEvaluationResults(object):
    ...
    def __init__(self, useOCLModel, soilFiles):
        ...
        :param useOCLModel: a valid UseOCLModelFile build with the analyzer.
        :type useOCLModel: analyzer.UseOCLModelFile
        :param soilFiles: list of paths to state files (.soil)
        :type soilFiles: [str]

        """
        assert isinstance(useOCLModel, UseOCLModelFile)
...
del OrderedDict, UseOCLModelFile, ModelValidation, InvariantViolation
